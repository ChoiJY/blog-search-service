...
import io.github.choijy.statisticsservice.configuration.EmbeddedRedisConfiguration;
...
class StatisticsServiceTest {
    ...
    class GetBlogSearchTop10Test {
        ...
        void getSearchHistoryTop10() {
            ...
            Set<TypedTuple<String>> result = new LinkedHashSet<>();
            result.add(new DefaultTypedTuple<>("카카오", 30.0));
            result.add(new DefaultTypedTuple<>("네이버", 23.0));
            ...
            List<SearchHistory> expected = Arrays.asList(new SearchHistory("카카오", 30),
                    new SearchHistory("네이버", 23));
        ...
        void getSearchHistoryTop10NoData() {
            when(redisTemplate.opsForZSet()).thenReturn(zSetOperations);
            when(zSetOperations.reverseRangeWithScores(rankingKey, 0, 10)).thenReturn(
        ...
        void getSearchHistoryTop10Failed() {
            when(redisTemplate.opsForZSet()).thenReturn(zSetOperations);
            when(zSetOperations.reverseRangeWithScores(rankingKey, 0, 10)).thenThrow(
    ...
    class IncreaseSearchHistoryCountTest {
        ...
        void increaseSearchHistoryCountFailed() {
            when(redisTemplate.opsForZSet()).thenReturn(zSetOperations);
            when(zSetOperations.incrementScore(rankingKey, searchKeyword, 1)).thenThrow(
                    new NullPointerException());
